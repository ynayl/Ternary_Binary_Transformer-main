...
def parse_args():
    ...
    # 动态学习率（ReduceLROnPlateau）相关参数
    parser.add_argument("--lr_patience", type=int, default=2, help="Patience for learning rate decay (ReduceLROnPlateau)")
    parser.add_argument("--lr_factor", type=float, default=0.5, help="Decay factor for learning rate (ReduceLROnPlateau)")
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        # 强制加载本地CSV文件，完全绕开官方数据集
        raw_datasets = load_dataset(
            "csv",  # 指定数据集格式为CSV
            data_files={
                "train": args.train_file,  # 本地训练集路径
                "validation": args.validation_file, # 本地验证集路径
                "test": "D:/Ternary_Binary_Transformer-main/dataset/cnn_dailymail/test_small.csv"  # 新增test分片
            },
            cache_dir=None  # 禁用缓存，避免读取旧数据
        )
    else:
        data_files = {}
        if args.train_file is not None:
    ...
    from py_rouge import PyRouge
    class LocalRougeMetric:
        def __init__(self):
            self.rouge = PyRouge(
                rouge_n=(1, 2),
                rouge_l=True,
        ...
        def add_batch(self, predictions, references):
            """适配脚本的add_batch调用，累加批量数据"""
            # 将当前批次的预测/参考文本存入列表
            self.predictions.extend(predictions)
            self.references.extend(references)

        def compute(self, use_stemmer=True):
            """统一计算所有批次的ROUGE分数（适配脚本的compute调用）"""
            # 处理空文本，避免计算报错
            preds = [str(p).strip() if str(p).strip() else "empty" for p in self.predictions]
            refs = [str(r).strip() if str(r).strip() else "empty" for r in self.references]

            # PyRouge要求的输入格式：multi_references是[[ref1], [ref2], ...]
            multi_references = [[ref] for ref in refs]

            # 计算ROUGE分数
            scores = self.rouge.evaluate(preds, multi_references)
            ...
            # 转换为脚本预期的输出格式
            return {
                "rouge1": {"f": scores["rouge-1"]["f"], "p": scores["rouge-1"]["p"], "r": scores["rouge-1"]["r"]},
                "rouge2": {"f": scores["rouge-2"]["f"], "p": scores["rouge-2"]["p"], "r": scores["rouge-2"]["r"]},
                "rougeL": {"f": scores["rouge-l"]["f"], "p": scores["rouge-l"]["p"], "r": scores["rouge-l"]["r"]},
                "rougeLsum": {"f": scores["rouge-l"]["f"], "p": scores["rouge-l"]["p"], "r": scores["rouge-l"]["r"]}
            }

    # 实例化metric
    metric = LocalRougeMetric()
    ...
    if args.do_train:
        for epoch in range(args.num_train_epochs):
            ...
            if args.output_dir is not None :
                ...
                # 2. 手动保存模型权重（避开配置保存的use_diff参数）
                weight_path = os.path.join(args.output_dir, WEIGHTS_NAME)  # 权重文件名：pytorch_model.bin
                accelerator.save(unwrapped_model.state_dict(), weight_path)
                print(f"模型权重已保存到：{weight_path}")

                # 3. 手动保存模型配置（4.17.0不支持use_diff，直接保存）
                config_path = os.path.join(args.output_dir, CONFIG_NAME)  # 配置文件名：config.json
                unwrapped_model.config.to_json_file(config_path)  # 无use_diff参数，兼容4.17.0
                ...
                print(f"训练参数已保存到：{training_args_path}")
    ...
    if args.do_test:
        ...
        if isinstance(list(result.values())[0], dict):
            # 若返回的是字典，直接提取fmeasure
            result = {key: value.get("fmeasure", 0) * 100 for key, value in result.items()}
        else:
            # 兼容原有对象格式，保留原有逻辑
            result = {key: value.mid.fmeasure * 100 for key, value in result.items()}
